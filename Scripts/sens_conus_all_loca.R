...
# Get reference raster to check for areas that should be land but are NA (ChatGPT suggestion)
reference_rast <- rast("F:/Data/LOCA2/monthly_tmax/ACCESS-CM2_r1_ssp245_tmax.tif")
reference_rast <- reference_rast[[1]] |>
  shift(dx = -360)
land_mask <- !is.na(reference_rast)

# For each raster, assign 100 to areas that should be land but are NA,
# and assign discrete classes
for (i in 1:9) {
  na_land <- is.na(sig_rast_list[[i]]) & land_mask
  na_land <- na_land[[1]]
  sig_rast_list[[i]][na_land] <- 100
