...
prism_rast <- terra::rast("F:/Data/PRISM_Rapp_prop.tif")
...
# Get LOCA files
loca_files <- list.files("F:/Data/LOCA2/regular_sap_days", full.names = TRUE)
loca_files <- loca_files[stringr::str_detect(loca_files, "prop")]
loca_files_short <- list.files("F:/Data/LOCA2/regular_sap_days", full.names = FALSE)
loca_files_short <- loca_files_short[stringr::str_detect(loca_files_short, "prop")]

# For each file...
for (i in seq_along(loca_files)) {
  # Load in raster
  loca_rast <- terra::rast(loca_files[i]) |>
    terra::shift(dx = -360)
  ...
  model <- stringr::str_extract(loca_files_short[i], "^[^_]+_") |>
    stringr::str_remove("_")
  scenario <- stringr::str_extract(loca_files_short[i], "ssp\\d{3}")

  # Add things to vectors
  model_names <- c(model_names, model)
  scenario_names <- c(scenario_names, scenario)
